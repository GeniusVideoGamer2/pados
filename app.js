...
function armBootGlitchAudio() {
  ...
  const audioButton = document.querySelector('.boot-audio-button');
  ...
  const startAudio = async () => {
    ...
    audioButton && (audioButton.textContent = 'GLITCH AUDIO ON');
    ...
    try {
      ...
      started = false;
      audioButton && (audioButton.textContent = 'TAP TO ENABLE AUDIO');
      return;
    }
    ...
    const scheduleTone = (time, frequency, duration, type = 'square', gain = 0.12) => {
    ...
    scheduleTone(now + 5.55, 170, 0.75, 'sawtooth', 0.15);
    scheduleTone(now + 8.05, 78, 0.55, 'square', 0.14);
    scheduleTone(now + 12.1, 520, 0.4, 'triangle', 0.1);
  ...
  audioButton?.addEventListener('click', startAudio, { once: true });
